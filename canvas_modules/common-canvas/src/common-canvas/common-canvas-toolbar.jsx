...
class CommonCanvasToolbar extends React.Component {
	...
	// and following divider passed in as that will be replaced by a
	...
	getNotificationClassName() {
		...
		let className = "fill " + NOTIFICATION_ICON_CLASS;
	...
	configureToolbarButtonsState(toolbarConfig) {
		...
		if (typeof toolbarConfig !== "undefined") {
			...
			this.toolFromConfig("undo", toolbarConfig).enable = undoState;
			this.toolFromConfig("redo", toolbarConfig).enable = redoState;
			this.toolFromConfig("cut", toolbarConfig).enable = cutState;
			this.toolFromConfig("copy", toolbarConfig).enable = copyState;
			this.toolFromConfig("paste", toolbarConfig).enable = pasteState;
			this.toolFromConfig("deleteSelectedObjects", toolbarConfig).enable = deleteState;
		}
		return toolbarConfig;
	}

	toolFromConfig(action, toolbarConfig) {
		let tool = {};
...
CommonCanvasToolbar.propTypes = {
	...
	isPaletteEnabled: PropTypes.object,
